...
export function MessageThread({
  ...
  if (!conversation || !contact) {
    return (
      <div className={cn("flex flex-1 flex-col items-center justify-center bg-slate-50", DOODLE_BG_CLASSES)}>
        <div className="flex h-16 w-16 items-center justify-center rounded-full bg-white shadow-sm border border-slate-200 text-slate-400">
  ...
  return (
    <div className={cn("flex flex-1 flex-col", DOODLE_BG_CLASSES)}>
      ...
      <div className="flex items-center justify-between gap-2 border-b border-slate-200 bg-white px-3 py-3 sm:px-4 shadow-xs">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          {onBack && (
            <button
              ...
              className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md text-slate-600 hover:bg-slate-100 hover:text-slate-900 lg:hidden"
            >
              <ArrowLeft className="h-5 w-5" />
            </button>
          )}
          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-slate-100 border border-slate-200 text-sm font-semibold text-slate-700">
            {displayName.charAt(0).toUpperCase()}
          </div>
          <div className="min-w-0">
            <h2 className="truncate text-sm font-semibold text-slate-900">{displayName}</h2>
            <p className="truncate text-xs text-slate-500 font-mono">{contact.phone}</p>
          </div>
          ...
          <Badge
            variant="outline"
            className={cn(
              "ml-1 hidden gap-1 border-slate-200 text-[10px] sm:inline-flex sm:ml-2 bg-slate-50",
              sessionInfo.expired ? "text-rose-600 border-rose-100 bg-rose-50" : "text-indigo-600 border-indigo-100 bg-indigo-50/50"
            )}
          >
        ...
        <div className="flex items-center gap-2">
          ...
          {onRefresh && (
            <button
              ...
              title="Refresh"
              className={cn(
                "inline-flex h-7 w-7 items-center justify-center rounded-md text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900 disabled:opacity-60",
              )}
            >
              <RefreshCw
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="border-slate-200 bg-white shadow-md text-slate-700"
            >
              {STATUS_OPTIONS.map((opt) => (
                <DropdownMenuItem
                  key={opt.value}
                  onClick={() => handleStatusChange(opt.value)}
                  className={cn("text-sm cursor-pointer focus:bg-slate-50", opt.value === "open" ? "text-indigo-600 focus:text-indigo-700" : opt.value === "pending" ? "text-amber-600 focus:text-amber-700" : "text-slate-500 focus:text-slate-600")}
                >
                  {opt.label}
                </DropdownMenuItem>
              ))}
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              className={cn(
                "inline-flex items-center justify-center h-7 gap-1 px-2 text-xs rounded-md hover:bg-slate-50",
                assignedAgentId ? "text-indigo-600 font-semibold bg-indigo-50/50" : "text-slate-500 hover:text-slate-700"
              )}
            >
                ...
                profiles.map((p) => {
                  ...
                  return (
                    <DropdownMenuItem
                      ...
                      className={cn(
                        "text-sm cursor-pointer focus:bg-slate-50",
              ...
              {assignedAgentId && (
                <>
                  ...
                  <DropdownMenuItem
                    ...
                    className="text-sm text-slate-500 hover:text-slate-700 cursor-pointer focus:bg-slate-50"
                  >
                    Unassign
                  </DropdownMenuItem>
                </>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <p className="text-sm text-slate-600 font-semibold">No messages yet</p>
            ...
            {messageGroups.map((group) => (
              <div key={group.date}>
                ...
                <div className="mb-4 flex items-center justify-center">
                  <span className="rounded-full bg-white shadow-xs border border-slate-200/60 px-3 py-1 text-[10px] font-medium text-slate-500">
