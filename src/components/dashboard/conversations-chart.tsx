  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-slate-200 bg-white shadow-sm">
      <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        <div>
          <h2 className="text-sm font-semibold text-slate-700">Conversations Over Time</h2>
      ...
      <footer className="flex items-center gap-4 border-t border-slate-200 px-5 py-3 text-xs text-slate-500">
        <LegendDot color="#3b82f6" label="Incoming" />
        <LegendDot color="#4f46e5" label="Outgoing" />
      </footer>
    </section>
  )
}
...
function LineSvg({
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      >
        ...
        {hover !== null && (
          <g pointerEvents="none">
            ...
            <circle cx={hoverX} cy={yFor(data[hover.idx].outgoing)} r={3.5} fill="#4f46e5" />
          </g>
        )}
      </svg>
      ...
      {hovered && hover !== null && (
        <div
          className="pointer-events-none absolute top-0 z-10 -translate-x-1/2 rounded-md border border-slate-200 bg-white px-2.5 py-1.5 text-[11px] shadow-lg"
          style={{ left: `${hover.tooltipLeftPx}px` }}
        >
          <div className="font-semibold text-slate-900">{longDayLabel(hovered.day)}</div>
