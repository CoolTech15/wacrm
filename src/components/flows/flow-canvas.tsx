...
function FlowNodeCard({ data, selected }: NodeProps) {
  ...
  return (
    <div
      className={cn(
        "relative min-w-[220px] max-w-[260px] rounded-lg border bg-white px-3 py-2 text-left shadow-md transition-colors",
        selected
    ...
    >
      {hasTarget && (
        <Handle
          ...
          className="!h-2.5 !w-2.5 !border-slate-300 !bg-slate-200"
        />
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        {isEntry && (
          <span className="ml-auto rounded bg-emerald-100 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-emerald-700">
            Entry
          </span>
        )}
      </div>
      ...
      {isMultiSlot && (
        <div className="mt-2 flex flex-col gap-1 border-t border-slate-100 pt-2">
          {slots.map((slot) => (
            <div
              key={slot.id}
              className="relative flex items-center justify-between gap-2 rounded px-1 py-0.5 text-[11px] text-slate-600"
            >
              ...
              <Handle
                ...
                className="!relative !right-auto !top-auto !h-2.5 !w-2.5 !translate-x-[12px] !transform-none !border-slate-300 !bg-slate-200"
              />
            </div>
          ))}
        </div>
...
function FlowCanvasInner() {
  ...
  if (rfNodes.length === 0) {
    return (
      <div className="flex h-[60vh] flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-slate-200 bg-slate-50 text-sm text-slate-400">
        <p>No nodes yet.</p>
        <CanvasAddNodeButton />
      </div>
    );
  ...
  return (
    <>
      <div className="h-[70vh] w-full overflow-hidden rounded-lg border border-slate-200 bg-slate-50 shadow-sm">
        <ReactFlow
          nodes={rfNodes}
          edges={rfEdges}
...
function NodeEditSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent
        side="right"
        className="flex w-full flex-col gap-0 border-l border-slate-200 bg-white p-0 sm:max-w-md"
      >
        ...
        <SheetFooter className="border-t border-slate-100 px-5 py-3 sm:flex-row sm:justify-between">
          {!isEntry ? (
...
function CanvasAddNodeButton() {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 shadow-md transition-colors hover:bg-slate-50"
        aria-label="Add node"
      >
