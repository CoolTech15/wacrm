...
export function ValidationPanel() {
  ...
  if (issues.length === 0) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm font-medium text-emerald-700 shadow-sm">
  ...
  return (
    <div
      className={cn(
        ...
        errors.length > 0 ? "border-red-200" : "border-amber-200",
      )}
    >
  ...
  const body = (
    <>
      ...
      <span className="min-w-0 flex-1">
        {issue.node_key && (
          <code className="mr-1 rounded bg-slate-100 px-1 py-0.5 text-[10px] text-slate-500 border border-slate-200">
            {issue.node_key}
          </code>
        )}
        {issue.message}
  ...
  if (issue.node_key && onJump) {
    return (
      <button
        ...
        className={cn(
          "flex w-full items-start gap-2 rounded-md px-2 py-1 text-left text-xs transition-colors hover:bg-slate-50",
          tone,
        )}
